...
def process_medals_and_update_file(athlete_event_file, paris_medallists, teams_dict, next_edition_id, athlete_birth_dict, games_start_dates_dict):
    ...
    # (code for adding/updating medallist rows )
    # ...

    # Adding the age column to the header if it doesn't exist
    if "age" not in athlete_event_file[0]:
        athlete_event_file[0].append("age")

    # Ensure each row has an "age" column
    for row in athlete_event_file[1:]:
        # If the row does not yet have the "age" field appended
        if len(row) < len(athlete_event_file[0]):
            row.append("")
...
def main():
    ...
    paris_athletes = read_csv_file("paris/medallists.csv")
    ...
    new_athlete_bio_file= read_csv_file("new_olympic_athlete_bio.csv")
    
    #Create the dicctionaries

    next_edition_id = calculate_next_edition_id(athlete_event_file)
    teams_dict = create_teams_dict(teams)
    athlete_birth_dict=create_athlete_birth_dict(new_athlete_bio_file)
